    ...
    <section className="relative mx-auto pb-12 overflow-hidden min-h-screen bg-black">
        ...
        <div className="relative z-10 space-y-6">
          <div className="space-y-6">
            ...
            <h1 className="text-4xl sm:text-6xl max-w-4xl mx-auto font-bold leading-[1.1] mb-4 font-[var(--font-inter-tight)]">
              ...
              <span className="text-[#ff6f00] ml-2">Get You Noticed</span>
            </h1>
            <p className="text-lg text-gray-300 max-w-3xl mx-auto mb-8">
              Boost credibility with polished, realistic headshots that match your professional brand. Perfect for LinkedIn, bios, and speaker pages.
            </p>
          </div>
          ...
          <div className="flex flex-col items-center space-y-2 pt-2">
            <div className="flex items-center space-x-2">
              <div className="flex -space-x-2">
                <img src="/content/sachin.webp" alt="User profile photo" className="w-8 h-8 rounded-full border-2 border-white object-cover" />
                <img src="/content/sumesh.webp" alt="User profile photo" className="w-8 h-8 rounded-full border-2 border-white object-cover" />
                <img src="/content/manoj.jpg" alt="User profile photo" className="w-8 h-8 rounded-full border-2 border-white object-cover" />
                <img src="/content/emma-thopmson.jpg" alt="User profile photo" className="w-8 h-8 rounded-full border-2 border-white object-cover" />
                <div className="w-8 h-8 rounded-full bg-gray-900 border-2 border-white flex items-center justify-center"><span className="text-white text-xs font-bold">1.2k</span></div>
              </div>
      ...
      <div className="w-full pt-4 sm:pt-12 overflow-hidden">
        <div className="hidden md:flex w-full">
          <div className="flex animate-slide-rtl">
            {[...images, ...images].map((image, index) => (
              <div key={index} className="min-w-[200px] p-2 relative flex-shrink-0" style={{ padding: '0 10px' }}>
                <div className="relative w-[200px] h-[280px]">
                  <Image src={image.src} alt={image.alt} fill className="rounded-lg object-cover" style={{ objectPosition: 'center' }} />
                  <div className="absolute bottom-2 left-2 flex items-center gap-[1.5px]"><Image src="/site-logo.png" alt="PFPforME Logo" width={24} height={24} className="w-6 h-6 rounded" /></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="block md:hidden w-full overflow-hidden">
          <div className="flex animate-slide-rtl-mobile" style={{ width: `${images.length * 2 * 220}px` }}>
            {images.map((image, index) => (
              <div key={`first-${index}`} className="w-[200px] p-2 relative flex-shrink-0">
                <div className="relative w-full h/full max-w-[200px] mx-auto aspect-[3/4]">
                  <Image src={image.src} alt={image.alt} fill className="rounded-lg object-cover" style={{ objectPosition: 'center top' }} />
                  <div className="absolute bottom-2 right-2 flex items-center gap-[1px] bg-white/80 rounded text-[10px]"><Image src="/site-logo.png" alt="PFPforME Logo" width={16} height={16} className="w-4 h-4 rounded" /><span className="px-1 py-0.5">AI</span></div>
                </div>
              </div>
            ))}
            {images.map((image, index) => (
              <div key={`second-${index}`} className="w-[200px] p-2 relative flex-shrink-0">
                <div className="relative w-full h/full max-w-[200px] mx-auto aspect-[3/4]">
                  <Image src={image.src} alt={image.alt} fill className="rounded-lg object-cover" style={{ objectPosition: 'center top' }} />
                  <div className="absolute bottom-2 right-2 flex items-center gap-[1px] bg-white/80 rounded text-[10px]"><Image src="/site-logo.png" alt="PFPforME Logo" width={16} height={16} className="w-4 h-4 rounded" /><span className="px-1 py-0.5">AI</span></div>
                </div>
      ...
      <style jsx>{`
        ...
        .animate-slide-rtl { animation: slide-rtl 60s linear infinite; }
        .animate-slide-rtl-mobile { animation: slide-rtl-mobile 38s linear infinite; }
        @keyframes slide-rtl-mobile { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
      `}</style>
      ...
      <VideoModal
        ...
        title="How It Works - LinkedIn Headshots Demo"
      />
    </section>
  )
}