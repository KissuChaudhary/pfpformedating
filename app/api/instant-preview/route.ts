...
// Preset styles for male-only previews
...
export async function POST(request: Request) {
  try {
    ...
    if (!styleKey || !STYLE_PROMPTS.man?.[styleKey]) {
      return NextResponse.json({ error: "Invalid style key", status: 400 }, { status: 400 });
    }
    ...
    const result = await fetch("https://fal.run/fal-ai/nano-banana/edit", {
      ...
      body: JSON.stringify({
        ...
        prompt: STYLE_PROMPTS.man[styleKey],
        aspect_ratio: "3:4",
        num_images: 1,
